...
--username ${user} \
--password ${password} \
--query "select id,customer_id,audit_remark,auditor,resource_id,audit_type,audit_time,state,audit_state,send_site_id,is_valid,sort_id,last_update_time,create_time,op_date,op_usr,op_ip,op_address,op_site_id,op_desc,audit_change_state,refuse_reason from log_customer_audit where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_customer_audit" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_log_customer_audit/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_log_customer_audit add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
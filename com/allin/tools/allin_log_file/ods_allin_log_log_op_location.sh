...
--username ${user} \
--password ${password} \
--query "select id,op_table,op_table_id,op_time,op_desc,op_ip,area_code,city,country_code,country_name,dma_code,latitude,longitude,metro_code,postal_code,region,ip_org,time_zone,create_time,send_site_id,op_advice from log_op_location where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_op_location" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_log_op_location/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_log_op_location add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
...
--username ${user} \
--password ${password} \
--query "select id,customer_id,send_scene_id,send_level_id,send_vm_name,send_account,send_content,send_subject,valid_code,send_time,create_time,send_status,callback_time,callback_mtstat,send_channel_id,is_valid from log_send_edm where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_send_edm" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_log_send_edm/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_log_send_edm add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
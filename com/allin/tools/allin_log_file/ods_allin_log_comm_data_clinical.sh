...
--username ${user} \
--password ${password} \
--query "select id,clinical_id,clinical_name,clinical_abstract,clinical_type,is_require,is_add,tree_level,is_leaf,full_path,parent_id,sort_id,create_time,site_id,is_valid,limit_term from comm_data_clinical where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from comm_data_clinical" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_comm_data_clinical/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_comm_data_clinical add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
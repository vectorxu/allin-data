...
--username ${user} \
--password ${password} \
--query "select id,customer_id,follow_type,ref_id,ref_name,follow_category,follow_time,follow_url,op_ip,op_address,is_valid,is_header,sort_id,sort_ref_name,op_date,op_usr,op_site_id,op_desc,is_permit,send_site_id from log_customer_follow_resource where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_customer_follow_resource" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_log_customer_follow_resource/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_log_customer_follow_resource add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_log_cms_video_history add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
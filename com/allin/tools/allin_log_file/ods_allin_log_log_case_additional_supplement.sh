...
--username ${user} \
--password ${password} \
--query "select id,additional_id,case_id,additional_type,content,sort_id,site_id,create_time,is_valid,op_date,op_usr,op_ip,op_address,op_desc from log_case_additional_supplement where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_case_additional_supplement" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_log_case_additional_supplement/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_log_case_additional_supplement add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
...
--username ${user} \
--password ${password} \
--query "select id,scene_type,resource_type,share_long_url,share_short_url,customer_id,send_site_id,create_time,is_valid from log_share where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_share" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_log_share/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_log_share add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
...
--username ${user} \
--password ${password} \
--query "select id,doc_id,doc_name,tag_id,tag_name,tag_parent_id,tag_full_path,sort_id,is_valid,create_time,send_site_id,op_date,op_usr,op_ip,op_address,op_desc from log_cms_doc_tag where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_cms_doc_tag" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_log_cms_doc_tag/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_log_cms_doc_tag add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
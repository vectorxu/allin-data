...
--username ${user} \
--password ${password} \
--query "select id,page_id,page_name,location_id,location_name,create_time,is_valid from log_track_page_location where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_track_page_location" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_log_track_page_location/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_log_track_page_location add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
...
--username ${user} \
--password ${password} \
--query "select id,movement_id,customer_id,movement_type,movement_content,ref_item_id,ref_item_type,ref_item_name,create_time,prefer_num,collection_num,review_num,share_num,reprint_num,update_time,is_valid,send_site_id,op_date,op_usr,op_ip,op_address,op_desc from log_customer_movement where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_customer_movement" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_log_customer_movement/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_log_customer_movement add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
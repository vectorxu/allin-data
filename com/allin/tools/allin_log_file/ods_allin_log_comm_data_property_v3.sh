...
--username ${user} \
--password ${password} \
--query "select id,property_type_id,property_id,property_name,parent_id,property_desc,tree_level,is_leaf,property_full_path,video_num,topic_num,doc_num,case_num,follow_num,create_time,sort_id,is_valid from comm_data_property_v3 where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from comm_data_property_v3" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_comm_data_property_v3/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_comm_data_property_v3 add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
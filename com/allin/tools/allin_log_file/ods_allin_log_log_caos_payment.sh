...
--username ${user} \
--password ${password} \
--query "select id,out_trade_no,trade_no,trade_status,notify_time,notify_type,seller_email,buyer_email,seller_id,buyer_id,price,total_fee,quantity,gmt_create,gmt_payment,gmt_close,verify_status,create_time,remark from log_caos_payment where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_caos_payment" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_log_caos_payment/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_log_caos_payment add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
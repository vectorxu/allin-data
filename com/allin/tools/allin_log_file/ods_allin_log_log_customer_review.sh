...
--username ${user} \
--password ${password} \
--query "select id,customer_id,parent_id,review_type,review_status,ref_id,ref_customer_id,ref_url,review_content,create_time,recommend_num,collection_num,review_num,reprint_num,up_num,down_num,op_ip,op_address,if_anonymous,if_header,is_valid,send_site_id,is_draft,op_date,op_desc,op_usr,cite_id,publish_time from log_customer_review where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_customer_review" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_log_customer_review/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_log_customer_review add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
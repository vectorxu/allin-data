...
--username ${user} \
--password ${password} \
--query "select id,video_id,video_att_type,video_att_size,video_att_format,video_att_spec,video_att_height,video_att_width,video_att_url,upload_time,sort_id,is_valid,send_site_id,op_date,op_usr,op_ip,op_address,op_desc,is_download,create_time from log_cms_video_attachment where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_cms_video_attachment" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_log_cms_video_attachment/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_log_cms_video_attachment add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
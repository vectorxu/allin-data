...
--username ${user} \
--password ${password} \
--query "select id,customer_id,session_id,site_id,site_source,search_page,search_word,search_time from log_search_word where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_search_word" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_log_search_word/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_log_search_word add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
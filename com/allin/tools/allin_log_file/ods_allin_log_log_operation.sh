...
--username ${user} \
--password ${password} \
--query "select id,user_id,user_name,page_id,system_id,menu_name,op_ip,op_class,op_method,op_advice,create_time,log_map from log_operation where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_operation" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_log_operation/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_log_operation add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
...
--username ${user} \
--password ${password} \
--query "select id,send_type,email,send_subject,send_content,send_time,send_channel_id,send_site_id,send_status,callback_time,callback_mtstat from log_email where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_email" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_log_email/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_log_email add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
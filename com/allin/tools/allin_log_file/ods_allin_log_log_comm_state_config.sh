...
--username ${user} \
--password ${password} \
--query "select id,config_id,state_type,state_type_name,state,state_name,email_title,email_content,create_time,sort_id,is_valid,op_date,op_user,op_ip,op_adress,op_desc from log_comm_state_config where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_comm_state_config" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_log_comm_state_config/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_log_comm_state_config add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
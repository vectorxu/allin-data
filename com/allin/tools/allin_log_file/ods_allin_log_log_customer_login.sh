...
--username ${user} \
--password ${password} \
--query "select id,customer_id,login_type,op_date,op_ip,op_address,op_source,op_desc,site_id from log_customer_login where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_customer_login" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_log_customer_login/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_log_customer_login add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
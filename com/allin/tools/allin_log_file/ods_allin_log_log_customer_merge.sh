...
--username ${user} \
--password ${password} \
--query "select id,main_customer_id,merge_customer_id,create_time,op_user,deal_time,merge_state,merge_type,merge_remark,send_site_id,is_valid,sort_id,op_date,op_ip,op_address,op_site_id,op_desc from log_customer_merge where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_customer_merge" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_log_customer_merge/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_log_customer_merge add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
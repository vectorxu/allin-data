...
--username ${user} \
--password ${password} \
--query "select id,customer_id,first_name,last_name,full_name,customer_email,customer_mobile,init_passwd,user_id,user_name,create_time,op_ip,send_site_id from log_reset_password where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_reset_password" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_log_reset_password/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_log_reset_password add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
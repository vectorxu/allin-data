...
--username ${user} \
--password ${password} \
--query "select userID,userName,userPassWord,userClass,materialClass,userRegisterDate,name,userSex,specialty,userAffiliation,address,country,zipCode,userTelephone,userMobile,userOicq,userPublish,userLoginNUM,lastLogin,description,registerIP,userMail,unite_allin_email,unite_allin_mobile,unite_caos_email,unite_caos_mobile from user_old where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_user_old/dt="${etl_date}" \
--num-mappers 1 \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_user_old add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
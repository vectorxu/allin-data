...
--username ${user} \
--password ${password} \
--query "select id,customer_id,language_id,language_name,award_year,sort_id,is_valid,send_site_id,language_flag,create_time,op_date,op_usr,op_ip,op_address,op_site_id,op_desc,cite_id from log_customer_language where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_customer_language" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_log_customer_language/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_log_customer_language add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
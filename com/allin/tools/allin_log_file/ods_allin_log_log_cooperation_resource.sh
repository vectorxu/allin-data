...
--username ${user} \
--password ${password} \
--query "select id,ref_customer_id,type_id,tpl_path,title_id,title,resource_abstract,resource_body,create_time,resource_status,is_valid from log_cooperation_resource where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_cooperation_resource" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_log_cooperation_resource/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_log_cooperation_resource add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
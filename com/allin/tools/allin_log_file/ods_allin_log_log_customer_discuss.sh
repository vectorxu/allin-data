...
--username ${user} \
--password ${password} \
--query "select id,ref_discuss_id,customer_id,parent_id,discuss_type,ref_id,discuss_content,publish_time,if_anonymous,recommend_num,collection_num,discuss_num,share_num,up_num,if_header,is_valid,op_date,op_usr,op_ip,op_address,op_source,op_desc from log_customer_discuss where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_customer_discuss" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_log_customer_discuss/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_log_customer_discuss add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
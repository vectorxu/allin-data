...
--username ${user} \
--password ${password} \
--query "select id,customer_id,download_type,resource_id,download_desc,start_time,end_time,create_time,site_id,is_valid from log_customer_download where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_customer_download" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_log_customer_download/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_log_customer_download add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
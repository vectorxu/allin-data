...
--username ${user} \
--password ${password} \
--query "select id,base_id,table_name,table_id,create_time,op_date,op_usr,op_ip,op_address,op_desc,table_content from train_base_log where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from train_base_log" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_train_base_log/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_train_base_log add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
...
--username ${user} \
--password ${password} \
--query "select id,customer_id,unite_id,unite_info,unite_follow_state,unite_follow_time,unite_openid,unite_uuid,unite_nickname,unite_sex,unite_language,country_id,country,province_id,province,city_id,city,district_id,district,unite_head_logo,unite_remark,is_valid,send_site_id,op_date,op_usr,op_ip,op_address,op_desc from log_customer_unite_info where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_customer_unite_info" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_log_customer_unite_info/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_log_customer_unite_info add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
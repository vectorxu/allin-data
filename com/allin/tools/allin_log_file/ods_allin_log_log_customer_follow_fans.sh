...
--username ${user} \
--password ${password} \
--query "select id,customer_id,fans_customer_id,fans_customer_name,follow_time,op_ip,op_address,is_valid,relationship,op_date,op_usr,op_site_id,op_desc,send_site_id,create_time,follow_type from log_customer_follow_fans where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_customer_follow_fans" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_log_customer_follow_fans/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_log_customer_follow_fans add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
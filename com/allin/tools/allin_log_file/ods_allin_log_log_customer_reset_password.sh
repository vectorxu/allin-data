...
--username ${user} \
--password ${password} \
--query "select id,customer_id,customer_email,email_content,reset_password_url,apply_source,apply_time,send_mail_time,confirm_time,is_valid,reset_site,valid_code,type_id,op_type,op_site,create_time from log_customer_reset_password where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_customer_reset_password" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_log_customer_reset_password/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_log_customer_reset_password add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
...
--username ${user} \
--password ${password} \
--query "select id,fellowship_id,fellowship_name,fellowship_sub_id,fellowship_sub_name,fellowship_sub_summary,start_time,end_time,create_time,sort_id,is_valid,op_date,op_usr,op_ip,op_address,op_site_id,op_desc,fellowship_sub_time from log_fellowship_sub where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_fellowship_sub" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_log_fellowship_sub/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_log_fellowship_sub add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
...
--username ${user} \
--password ${password} \
--query "select id,resource_id,resource_type,page_storage_path,web_storage_path,share_page_short_url,share_web_short_url,create_time,is_valid,scene_type from log_short_url_history where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_short_url_history" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_log_short_url_history/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_log_short_url_history add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
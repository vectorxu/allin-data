...
--username ${user} \
--password ${password} \
--query "select id,customer_id,session_id,search_type,search_url,key_word,create_time,op_ip,op_address,send_site_id,is_valid from log_customer_keyword_20180702 where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_customer_keyword_20180702" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_log_customer_keyword_20180702/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_log_customer_keyword_20180702 add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
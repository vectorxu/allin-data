...
--username ${user} \
--password ${password} \
--query "select id,fellowship_id,fellowship_name,fellowship_major_id,fellowship_department,tag_id,tag_name,fellowship_date,fellowship_time,fellowship_describe,start_time,end_time,create_time,sort_id,is_valid,op_date,op_usr,op_ip,op_address,op_site_id,op_desc from log_fellowship_major where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_fellowship_major" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_log_fellowship_major/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_log_fellowship_major add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
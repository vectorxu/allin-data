...
--username ${user} \
--password ${password} \
--query "select id,ref_news_id,owner_channel_id,owner_column_id,news_title,news_abstract,news_body,news_status,sort_id,is_valid,storage_path,op_date,op_usr,op_ip,op_address,op_source,op_desc from log_cms_news where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_cms_news" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_log_cms_news/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_log_cms_news add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
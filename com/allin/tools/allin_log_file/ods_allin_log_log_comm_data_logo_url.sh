...
--username ${user} \
--password ${password} \
--query "select id,ref_id,logo_type,logo_spec,logo_height,logo_width,logo_size,logo_url,sort_id,is_valid,create_time,dyna_height,dyna_width,op_date,op_usr,op_ip,op_address,op_site_id,op_desc,send_site_id,cite_id from log_comm_data_logo_url where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_comm_data_logo_url" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_log_comm_data_logo_url/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_log_comm_data_logo_url add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
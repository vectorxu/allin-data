...
--username ${user} \
--password ${password} \
--query "select id,customer_id,browse_type,browse_url,open_time,leave_time,op_ip,op_address,op_source,op_desc,is_valid,send_site_id,device_token from log_customer_browse where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_customer_browse" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_log_customer_browse/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_log_customer_browse add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
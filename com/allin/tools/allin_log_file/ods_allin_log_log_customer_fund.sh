...
--username ${user} \
--password ${password} \
--query "select id,customer_id,fund_code,fund_name,approval_time,start_time,end_time,up_now,create_time,sort_id,is_valid,op_date,op_usr,op_ip,op_address,op_site_id,op_desc,send_site_id,language_flag,cite_id from log_customer_fund where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_customer_fund" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_log_customer_fund/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_log_customer_fund add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
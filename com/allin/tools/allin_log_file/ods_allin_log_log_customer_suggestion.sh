...
--username ${user} \
--password ${password} \
--query "select id,customer_id,customer_name,site_id,suggestion,system_version,equipment_version,network_environment,create_time,suggestion_status,suggestion_type,op_date,op_usr,op_name,op_desc,remark,is_valid,send_site_id,state,suggestion_id,browse_type from log_customer_suggestion where STR_TO_DATE(create_time,'%Y-%m-%d')='"""${etl_date}"""' and 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_customer_suggestion" \
--delete-target-dir \
--target-dir /user/hive/data/ods/allin_platform_log/ods_allin_log_log_customer_suggestion/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_allin_log_log_customer_suggestion add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
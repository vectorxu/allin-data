...
--username ${user} \
--password ${password} \
--query "select id,customer_id,browse_type,browse_url,open_time,leave_time,op_ip,op_address,op_source,op_desc,is_valid,send_site_id,device_token,resource_id,remarkInfo,op_advice,session_id,app_version from log_med_customer_browse where 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_med_customer_browse" \
--delete-target-dir \
--target-dir /user/hive/data/ods/med_platform_log/ods_log_med_customer_browse/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_log_med_customer_browse add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
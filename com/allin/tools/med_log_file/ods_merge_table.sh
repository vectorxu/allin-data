...
--username ${user} \
--password ${password} \
--query "select id,allin_id,allin_customer_id,med_id,med_customer_id,is_valid,id,allin_id,allin_customer_id,med_id,med_customer_id,is_valid from merge_table where 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from merge_table" \
--delete-target-dir \
--target-dir /user/hive/data/ods/med_platform_log/ods_merge_table/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_merge_table add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
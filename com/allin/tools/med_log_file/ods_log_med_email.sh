...
--username ${user} \
--password ${password} \
--query "select id,send_type,email,send_subject,send_content,send_time,send_channel_id,site_id,send_status,callback_time,callback_mtstat from log_med_email where 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_med_email" \
--delete-target-dir \
--target-dir /user/hive/data/ods/med_platform_log/ods_log_med_email/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_log_med_email add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
...
--username ${user} \
--password ${password} \
--query "select id,con_id,con_sub_id,con_type,con_name,con_content,con_place,moderator,reach_date,start_time,end_time,lecturer,topic,ref_video_id,is_online,sort_id,is_valid,live_id,play_back_id from conference_agenda where 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from conference_agenda" \
--delete-target-dir \
--target-dir /user/hive/data/ods/med_platform_log/ods_conference_agenda/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_conference_agenda add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
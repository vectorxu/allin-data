...
--username ${user} \
--password ${password} \
--query "select id,send_site_id,device_token,content,source_id,source_type,message_first_result,badge,send_map,create_time from log_med_notify_message where 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_med_notify_message" \
--delete-target-dir \
--target-dir /user/hive/data/ods/med_platform_log/ods_log_med_notify_message/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_log_med_notify_message add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
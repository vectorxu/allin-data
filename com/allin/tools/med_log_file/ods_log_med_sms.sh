...
--username ${user} \
--password ${password} \
--query "select id,cell_phone,sms_content,mtmsgid,send_time,send_mtstat,send_mterrcode,callback_time,callback_mtstat,callback_mterrcode,sms_desc from log_med_sms where 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_med_sms" \
--delete-target-dir \
--target-dir /user/hive/data/ods/med_platform_log/ods_log_med_sms/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_log_med_sms add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
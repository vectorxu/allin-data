    ...
    for row in results:
        ...
        if ('id' in l):
            addpar = \
                ...
                "--username ${user} \\\n" \
                "--password ${password} \\\n" \
                "--query \"select " + col + " from " + tablename + " where STR_TO_DATE(create_time,'%Y-%m-%d')=\'"+"\"\"\"${etl_date}\"\"\"\' and 1=1 AND \\$CONDITIONS\" \\\n" \
                "--split-by id \\\n" \
...
def get_colume(tablename):
   ...
   db1 = pymysql.connect(host="101.201.140.170", user="dev", password="vXKGQ_7WrjVHRla8", port=4417,database="information_schema")
   cur = db1.cursor()
...
def write_file(filename,file):
   path = "/Users/allin/PycharmProjects/allin-data/com/allin/tools/allin_log_file/"+filename

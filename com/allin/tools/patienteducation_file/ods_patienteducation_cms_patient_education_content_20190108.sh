...
--username ${user} \
--password ${password} \
--query "select id,education_id,education_content,create_time,tpl_path,sort_id,is_valid,send_site_id,content_type,content_font_type,content_att_name,content_item_symbol,content_att_id from cms_patient_education_content_20190108 where 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from cms_patient_education_content_20190108" \
--delete-target-dir \
--target-dir /user/hive/data/ods/patienteducation_platform/ods_patienteducation_cms_patient_education_content_20190108/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_patienteducation_cms_patient_education_content_20190108 add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
...
--username ${user} \
--password ${password} \
--query "select id,resource_id,qr_type,scene_type,qr_code_url,qr_content,ref_customer_type,ref_customer_id,resource_type,is_valid,site_id,create_time from cms_resource_qr_code_20190315 where 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from cms_resource_qr_code_20190315" \
--delete-target-dir \
--target-dir /user/hive/data/ods/patienteducation_platform/ods_patienteducation_cms_resource_qr_code_20190315/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_patienteducation_cms_resource_qr_code_20190315 add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
...
--username ${user} \
--password ${password} \
--query "select id,education_id,education_type,att_name,att_type,att_size,att_format,att_spec,att_height,att_width,att_url,upload_time,sort_id,is_valid,dyna_height,dyna_width,send_site_id,create_time,qiniu_id,qiniu_status,is_qiniu,play_num,play_time,att_source from cms_patient_education_attachment where 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from cms_patient_education_attachment" \
--delete-target-dir \
--target-dir /user/hive/data/ods/patienteducation_platform/ods_patienteducation_cms_patient_education_attachment/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_patienteducation_cms_patient_education_attachment add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
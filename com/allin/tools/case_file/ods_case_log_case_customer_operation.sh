...
--username ${user} \
--password ${password} \
--query "select id,customer_id,customer_name,case_id,structure_id,structure_name,create_time,op_ip,op_address,op_type,is_valid,site_id from log_case_customer_operation where 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_case_customer_operation" \
--delete-target-dir \
--target-dir /user/hive/data/ods/case_platform/ods_case_log_case_customer_operation/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_case_log_case_customer_operation add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
...
--username ${user} \
--password ${password} \
--query "select id,dictionary_id,dictionary_type,dictionary_name,dictionary_name_en,normal_name,normal_code,group_name,parent_id,dictionary_desc,tree_level,is_leaf,dictionary_full_path,create_time,update_time,site_id,sort_id,is_valid from comm_data_dictionary where 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from comm_data_dictionary" \
--delete-target-dir \
--target-dir /user/hive/data/ods/case_platform/ods_case_comm_data_dictionary/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_case_comm_data_dictionary add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
...
--username ${user} \
--password ${password} \
--query "select id,resource_id,resource_type,op_usr,op_ip,op_address,op_desc,op_param,op_date,site_id,create_time,sort_id,is_valid from case_folder_process where 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from case_folder_process" \
--delete-target-dir \
--target-dir /user/hive/data/ods/case_platform/ods_case_case_folder_process/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_case_case_folder_process add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
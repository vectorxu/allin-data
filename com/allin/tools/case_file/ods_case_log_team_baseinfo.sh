...
--username ${user} \
--password ${password} \
--query "select id,team_id,team_name,creator_id,customer_id,qr_code_url,is_apply,is_transfer,sort_id,site_id,create_time,update_time,is_valid,op_date,op_usr,op_ip,op_desc from log_team_baseinfo where 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_team_baseinfo" \
--delete-target-dir \
--target-dir /user/hive/data/ods/case_platform/ods_case_log_team_baseinfo/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_case_log_team_baseinfo add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
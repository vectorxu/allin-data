...
--username ${user} \
--password ${password} \
--query "select id,case_id,detail_id,structure_id,casefolder_video_id,group_id,att_type,att_size,att_format,att_spec,att_height,att_width,att_url,sort_id,site_id,qiniu_id,qiniu_status,is_qiniu,play_num,play_time,create_time,update_time,is_valid,remarks from case_folder_attachment where 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from case_folder_attachment" \
--delete-target-dir \
--target-dir /user/hive/data/ods/case_platform/ods_case_case_folder_attachment/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_case_case_folder_attachment add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
...
--username ${user} \
--password ${password} \
--query "select id,customer_id,team_id,customer_state,customer_role,sort_id,site_id,create_time,update_time,is_valid,op_date,op_usr,op_ip,op_desc from log_team_member where 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from log_team_member" \
--delete-target-dir \
--target-dir /user/hive/data/ods/case_platform/ods_case_log_team_member/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_case_log_team_member add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
...
--username ${user} \
--password ${password} \
--query "select id,field_id,dictionary_id,field_name,field_type,group_label,field_desc,create_time,update_time,site_id,sort_id,is_valid from comm_data_field where 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from comm_data_field" \
--delete-target-dir \
--target-dir /user/hive/data/ods/case_platform/ods_case_comm_data_field/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_case_comm_data_field add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
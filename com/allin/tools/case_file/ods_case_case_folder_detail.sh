...
--username ${user} \
--password ${password} \
--query "select id,detail_id,case_id,structure_id,structure_type,content,other_content,group_id,tree_level,parent_id,site_id,sort_id,create_time,update_time,is_valid,side_type from case_folder_detail where 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from case_folder_detail" \
--delete-target-dir \
--target-dir /user/hive/data/ods/case_platform/ods_case_case_folder_detail/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_case_case_folder_detail add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
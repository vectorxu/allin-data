...
--username ${user} \
--password ${password} \
--query "select id,case_id,template_id,belong_type,team_id,customer_id,patient_id,patient_name,patient_age,patient_sex,patient_mobile,number_type,number_content,sort_id,create_time,update_time,site_id,is_valid,version from case_folder_baseinfo where 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from case_folder_baseinfo" \
--delete-target-dir \
--target-dir /user/hive/data/ods/case_platform/ods_case_case_folder_baseinfo/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_case_case_folder_baseinfo add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
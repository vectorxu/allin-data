...
--username ${user} \
--password ${password} \
--query "select id,template_id,relation_type,team_id,customer_id,create_time,update_time,site_id,sort_id,is_valid from case_folder_team_relation where 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from case_folder_team_relation" \
--delete-target-dir \
--target-dir /user/hive/data/ods/case_platform/ods_case_case_folder_team_relation/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_case_case_folder_team_relation add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
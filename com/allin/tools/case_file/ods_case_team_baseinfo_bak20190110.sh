...
--username ${user} \
--password ${password} \
--query "select id,team_id,team_name,creator_id,customer_id,qr_code_url,is_apply,is_transfer,sort_id,site_id,create_time,update_time,is_valid from team_baseinfo_bak20190110 where 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from team_baseinfo_bak20190110" \
--delete-target-dir \
--target-dir /user/hive/data/ods/case_platform/ods_case_team_baseinfo_bak20190110/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_case_team_baseinfo_bak20190110 add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
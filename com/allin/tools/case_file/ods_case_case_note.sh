...
--username ${user} \
--password ${password} \
--query "select id,note_id,note_name,customer_id,sort_id,is_valid,site_id,create_time,update_time from case_note where 1=1 AND \$CONDITIONS" \
--split-by id \
--boundary-query "select min(id),max(id) from case_note" \
--delete-target-dir \
--target-dir /user/hive/data/ods/case_platform/ods_case_case_note/dt="${etl_date}" \
--null-string '\\N' \
--null-non-string '\\N' \
...
if [ $? -eq 0 ];then
    hive -e "alter table ods.ods_case_case_note add if not exists partition(dt='"""${etl_date}"""');"
else
    exit 1
fi
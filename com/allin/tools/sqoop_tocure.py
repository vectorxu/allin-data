...
# 生成sqoop语句
def get_table():
    db = pymysql.connect(host="wwwhuizhen.mysql.rds.aliyuncs.com", user="dev", password="pTVunauXhSDx2Gg2", port=3317,database="information_schema")
    ...
    for row in results:
        ...
        addpar=\
           ...
           "\n" \
           "host=`sudo cat sqoop_tocure_to_ods.conf | grep host | awk -F'=' '{ print $2 }' | sed s/[[:space:]]//g`\n"\
           "port=`sudo cat sqoop_tocure_to_ods.conf | grep port | awk -F'=' '{ print $2 }' | sed s/[[:space:]]//g`\n"\
           "user=`sudo cat sqoop_tocure_to_ods.conf | grep user | awk -F'=' '{ print $2 }' | sed s/[[:space:]]//g`\n" \
           "password=`sudo cat sqoop_tocure_to_ods.conf | grep password | awk -F'=' '{ print $2 }' | sed s/[[:space:]]//g`\n" \
           "database=`sudo cat sqoop_tocure_to_ods.conf | grep database | awk -F'=' '{ print $2 }' | sed s/[[:space:]]//g`\n"\
           "\n"\
           "sqoop import \\\n"\
...
def get_colume(tablename):
   ...
   db1 = pymysql.connect(host="wwwhuizhen.mysql.rds.aliyuncs.com", user="dev", password="pTVunauXhSDx2Gg2", port=3317,
...
def write_file(filename,file):
   path = "/Users/allin/PycharmProjects/allin-data/com/allin/tools/tocure_file/"+filename
